...
## Project Overview
**Highlight Processor** — a local video processing pipeline for creating Instagram Reels from gameplay/highlight clips. Runs entirely via Docker Compose.

**Pipeline:** Raw video → crop to 9:16 (1080×1920) → remove vocals (Spleeter AI) → upload to Instagram Reels (Graph API).
...
- **instagram_upload** — Upload service using same image
...
- **No external HTTP library:** `urllib.request` only (no `requests`)
